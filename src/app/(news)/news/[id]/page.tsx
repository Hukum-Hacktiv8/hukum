...
export default function NewsArticle({ params }: { params: { id: string } }) {
  const [article, setArticle] = useState<Article>({
    ...
    subtitle: "Regulasi baru membawa angin segar bagi pelaku usaha kecil & menengah dlm pendirian & pengelolaan PT",
    content: `
        ...
        <div className="grid grid-cols-12 gap-8">
          ...
          <div className="col-span-12 lg:col-span-8">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-slate-800 rounded-xl overflow-hidden">
              {/* Article Header */}
              <div className="p-6">
                <span className="inline-block px-3 py-1 bg-yellow-500/10 text-yellow-500 text-sm font-medium rounded-lg mb-4">{article.category}</span>
                ...
                <div className="flex items-center justify-between border-t border-slate-700 pt-6">
                  <div className="flex items-center gap-4">
                    <Image src={article.author.avatar} alt={article.author.name} width={48} height={48} className="w-12 h-12 rounded-full" unoptimized />
                    <div>
              ...
              <div className="relative aspect-[21/9] w-full">
                <Image src={article.thumbnail} alt={article.title} fill className="object-cover" priority unoptimized />
              </div>
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-4">
                    <div className="flex flex-wrap gap-2">
                      {article.tags.map((tag, idx) => (
                        <Link key={idx} href={`/news/tag/${tag.toLowerCase()}`} className="px-3 py-1 bg-slate-700 text-sm text-gray-300 rounded-lg hover:bg-slate-600">
                          #{tag}
                        </Link>
                      ))}
                    </div>
                ...
                {article.relatedArticles.map((related) => (
                  <Link key={related.id} href={`/news/${related.id}`} className="group block">
                    <div className="relative aspect-[16/9] rounded-lg overflow-hidden mb-3">
                      <Image src={related.thumbnail} alt={related.title} fill className="object-cover group-hover:scale-105 transition-transform duration-300" unoptimized />
                    </div>
                    <span className="text-sm text-yellow-500">{related.category}</span>
                    <h4 className="text-white font-medium group-hover:text-yellow-500 transition-colors line-clamp-2">{related.title}</h4>
                  </Link>
                ))}
              </div>
            </div>
