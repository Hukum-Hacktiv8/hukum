...
import { IoPersonOutline, IoLogOutOutline, IoCalendarOutline, IoBookmarkOutline, IoChevronForward, IoCamera, IoHomeOutline, IoNotificationsOutline, IoWalletOutline, IoHelpCircleOutline } from "react-icons/io5";
            ...
            <div className="col-span-12 md:col-span-3">
              <div className="bg-slate-800 rounded-xl p-6 sticky top-20 max-h-[calc(100vh-8rem)]">
                ...
                <nav className="space-y-2">
                  {menuItems.map((item) => (
                    <button key={item.id} onClick={() => setActiveTab(item.id as any)} className={`w-full flex items-center gap-3 px-4 py-2 rounded-lg transition-colors ${activeTab === item.id ? "bg-yellow-500 text-slate-900" : "text-gray-300 hover:bg-slate-700 hover:text-white"}`}>
                      {item.icon}
