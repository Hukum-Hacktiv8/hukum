      ...
      <div className="container mx-auto px-4 mb-12">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredLawyers.map((lawyer, idx) => (
            <motion.div key={lawyer.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.1 }} className="bg-slate-800 rounded-xl overflow-hidden hover:bg-slate-800/80 transition-colors">
              <div className="p-6">
                {/* Lawyer Info */}
                ...
                <div className="flex items-center justify-between pt-4 border-t border-slate-700">
                  <div>
                    ...
                    <div className="text-green-500 text-sm">{lawyer.availability}</div>
                  </div>
                  <div className="flex items-center gap-2">
                    <button onClick={() => (window as any).scrollToLawyer(lawyer.id)} className="p-2 text-yellow-500 hover:bg-yellow-500/10 rounded-lg transition-colors" title="Lihat di Map">
                      <IoLocationOutline className="w-6 h-6" />
                    </button>
                    <Link href={`/booking/${lawyer.id}`} className="px-4 py-2 bg-yellow-500 hover:bg-yellow-600 text-slate-900 font-medium rounded-lg transition-colors">
                      Book Now
                    </Link>
                  </div>
                </div>
