                ...
                if (slot.available) {
                    ...
                    for (let i = 1; i < selectedDuration; i++) {
                        const nextSlot = slots.find(s => 
                            parseInt(s.time.split(":")[0]) === slotHour + i
                        )
                                    ...
                                    <div className="flex overflow-x-auto hide-scrollbar gap-3 px-1 py-2">
                                        {getDates().map((date, idx) => (
                                            <button
                                                ...
                                                className={`flex-shrink-0 flex flex-col items-center p-4 rounded-xl transition-colors ${
                                                    isDateSelected(date)
                                ...
                                <div className="mb-8">
                                        ...
                                        {timeSlots.map((slot, idx) => (
                                            <button
                                                ...
                                                className={`p-3 rounded-lg text-center transition-colors ${
                                                    !slot.available
                                                        ? "bg-slate-700/50 text-gray-500 cursor-not-allowed relative group"
                                            ...
                                            >
                                                ...
                                                {!slot.available && slot.bookedDuration && (
                                                    <span className="absolute hidden group-hover:block bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 text-xs text-white bg-slate-600 rounded">
                                                        Booked for {slot.bookedDuration}h
                                                    </span>
                                                )}
                                            </button>
                                        ))}
                                    ...
                                    {selectedDuration > 1 && (
                                        <p className="mt-3 text-sm text-gray-400">
                                            *Menampilkan slot yg tersedia utk durasi {selectedDuration} jam
                                        </p>
                                    )}
                                </div>
                                        ...
                                        <div className="space-y-3 mb-6">
                                            ...
                                            <div className="pt-3 border-t border-slate-600">
                                                <div className="flex justify-between text-white font-semibold">
                                                    <span>Total</span>
                                                    <span>
                                                        Rp {(lawyer.price * selectedDuration).toLocaleString('id-ID')}
                                                    </span>
                                                </div>
                                            </div>
                                        </div>
                                        <button className="w-full px-6 py-3 bg-yellow-500 hover:bg-yellow-600 text-slate-900 font-semibold rounded-lg transition-colors">
                                            Konfirmasi Booking
                                        </button>
                                    </div>
                                )}
